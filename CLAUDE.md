...
- `POST /rates` — confirmed working request shape (from Easyship's own generated Swift sample):
  ...
  - **Customs classification is validated in two passes, and they disagree about the field name.** Pass one: `"parcels[0].items[0].category can't be blank if hs_code is blank"`. Get past that by sending `category: "fashion"` and pass two fires: `"Shipment items need to input either item_category_id or hs_code at least"`. So the plain `category` string is **not sufficient** — one of `item_category_id` or `hs_code` is genuinely required. Both messages came from live 422s. The app sends `category` (as `ParcelItem.defaultCategory`, harmless and satisfies pass one) *plus* whichever of `hs_code` / `item_category_id` the user supplies, and the UI leads with `hs_code` since it's the one a user can look up. Valid `item_category_id` values are unpublished — `developers.easyship.com` renders via JavaScript **and** is blocked by the agent sandbox's egress proxy, so don't try to fetch it.
  - **Item dimensions are required, not optional.** A rate request whose item carries no `dimensions` is rejected. They were behind an "Add dimensions" disclosure button until a live 422 said otherwise. `RateCalculatorViewModel.missingRequirements` now enforces weight, all three dimensions, and the customs field before submit, so each rule surfaces in the UI rather than as a server error.
...
- Parcel weight *and* all three dimensions are required (see the API notes above) — don't be tempted to hide dimensions behind a disclosure again.
...
- `ParcelItem.category` — only `"fashion"` is confirmed valid. Grab the real enum from the API reference's "Try It" panel in a browser and turn the free-text field into a picker; until then a wrong value 422s (legibly, at least).
