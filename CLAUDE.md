...
- `POST /rates` — confirmed working request shape (from Easyship's own generated Swift sample):
  ...
  - **`items[]` requires one of `category` or `hs_code`** — confirmed by a live 422: `"parcels[0].items[0].category can't be blank if hs_code is blank"` / `"…hs_code can't be blank if category is blank"`. The valid `category` enum is still unpublished: the API reference renders via JavaScript and `developers.easyship.com` is unreachable from the agent sandbox, so don't try to fetch it. `"fashion"` is the one value confirmed to pass (from Easyship's own generated sample) and is `ParcelItem.defaultCategory`, pre-filled into an editable field. `hs_code` (eight-digit WTO code) is the always-valid alternative. `RateCalculatorViewModel.missingRequirements` enforces the either/or before submit so the rule surfaces in the UI rather than as a 422.
